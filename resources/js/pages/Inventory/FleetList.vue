...
<template>
        ...
        <div class="max-w-7xl mx-auto">
            <div class="bg-white p-6 rounded-3xl shadow-sm border border-gray-200 mb-6 flex flex-col md:flex-row justify-between items-center gap-4">
                ...
                <div class="relative w-full md:w-64">
                    <input v-model="searchQuery" placeholder="Search rigs..." class="w-full rounded-xl border-gray-200 text-sm px-4 py-2 focus:ring-2 focus:ring-indigo-500" />
                </div>
            </div>
                ...
                <div class="overflow-x-auto">
                    <table class="w-full text-left border-collapse">
                        ...
                        <tbody class="divide-y divide-gray-100 text-sm">
                            <tr v-for="item in equipment.data" :key="item.id" class="hover:bg-gray-50/50 transition-colors">
                                <td class="px-6 py-4">
                                    ...
                                    <div v-if="item.spares?.length > 0" class="mt-1">
                                        <span class="text-[8px] bg-indigo-50 text-indigo-600 px-2 py-0.5 rounded-full font-bold uppercase border border-indigo-100">
                                            {{ item.spares.length }} Spares Attached
                                        </span>
                                    </div>
                                </td>
                                <td class="px-6 py-4">
                                    <span class="text-xs block mb-1 font-bold text-gray-700">{{ item.current_site?.location_name || 'In Transit' }}</span>
                                    <select @change="handleTransfer(item.id, $event)" class="text-[9px] border-none bg-gray-100 rounded-lg p-1 focus:ring-1 focus:ring-indigo-400">
                                        <option value="">Move...</option>
                                        <option v-for="site in sites" :key="site.id" :value="site.id">{{ site.location_name }}</option>
                                    </select>
                                </td>
                                <td class="px-6 py-4">
                                    <span :class="getStatusBadge(item.status)" class="px-3 py-1 rounded-full text-[9px] font-black uppercase border shadow-sm">
                                        {{ item.status }}
                                    </span>
                                </td>
    ...
    <div v-if="showLogsModal && selectedItem" class="fixed inset-0 bg-slate-900/70 flex items-center justify-center z-50 p-4 backdrop-blur-sm">
        <div class="bg-white rounded-[2rem] shadow-2xl w-full max-w-2xl max-h-[85vh] overflow-y-auto p-8 border border-white/20">
            <div class="flex justify-between items-center mb-8 border-b pb-4">
                ...
                <button @click="showLogsModal = false" class="text-3xl font-light hover:text-red-500 transition-colors">&times;</button>
            </div>

            <div class="mb-10">
                <h4 class="text-[10px] font-black text-gray-400 uppercase tracking-widest mb-4 italic">Attached Spares ({{ selectedItem.spares?.length || 0 }})</h4>
                <div v-if="selectedItem.spares?.length > 0" class="grid grid-cols-1 md:grid-cols-2 gap-3">
                    <div v-for="spare in selectedItem.spares" :key="spare.id" class="p-4 bg-indigo-50/50 border border-indigo-100 rounded-2xl flex justify-between items-center">
                        <div>
                            <p class="font-bold text-xs text-gray-800">{{ spare.name }}</p>
                            <p class="text-[9px] font-mono text-indigo-400 font-bold uppercase">{{ spare.serial_number }}</p>
                        </div>
                        <button @click="unlinkSpare(spare.id)" class="text-[9px] font-black text-red-500 uppercase underline hover:text-red-700">Unlink</button>
                    </div>
                </div>
                <div v-else class="text-center py-6 bg-gray-50 rounded-3xl border border-dashed text-gray-400 text-[10px] font-bold uppercase">
                    No Spares Attached
                </div>
            </div>
            ...
            <div class="mb-6">
                <div class="flex justify-between items-center mb-4">
                    <h4 class="text-[10px] font-black text-gray-400 uppercase tracking-widest italic">Service History</h4>
                    <button @click="showLogForm = !showLogForm" class="bg-indigo-600 text-white px-3 py-1 rounded-lg text-[9px] font-black uppercase tracking-tighter hover:bg-indigo-700 shadow-md transition-all">
                        {{ showLogForm ? 'Cancel' : '+ Add Record' }}
                    </button>
                </div>

                <div v-if="showLogForm" class="bg-gray-50 p-6 rounded-3xl border border-dashed border-indigo-200 mb-6 shadow-inner">
                    <form @submit.prevent="submitLog" class="grid grid-cols-2 gap-4">
                        <div class="col-span-1">
                            ...
                            <input v-model="logForm.service_date" type="date" class="w-full border-gray-200 rounded-xl text-xs focus:ring-indigo-500" required />
                        </div>
                        <div class="col-span-1">
                            <label class="text-[9px] font-bold uppercase text-gray-400 ml-2">Mechanic</label>
                            <select v-model="logForm.mechanic_id" class="w-full border-gray-200 rounded-xl text-xs focus:ring-indigo-500" required>
                        ...
                        <div class="col-span-1">
                            ...
                            <input v-model="logForm.running_hours" type="number" step="0.1" class="w-full border-gray-200 rounded-xl text-xs focus:ring-indigo-500" required />
                        </div>
                        <div class="col-span-1">
                            <label class="text-[9px] font-bold uppercase text-gray-400 ml-2">Service Type</label>
                            <input v-model="logForm.service_type" placeholder="e.g. 1000H Service" class="w-full border-gray-200 rounded-xl text-xs focus:ring-indigo-500" required />
                        </div>
                        <div class="col-span-2">
                            <label class="text-[9px] font-bold uppercase text-gray-400 ml-2">Maintenance Details</label>
                            <textarea v-model="logForm.parts_replaced" placeholder="Describe parts replaced or work done..." class="w-full border-gray-200 rounded-xl text-xs focus:ring-indigo-500" rows="2"></textarea>
                        </div>
                        <button type="submit" class="col-span-2 py-3 bg-indigo-600 text-white rounded-xl font-black uppercase text-[10px] hover:bg-indigo-700 shadow-lg transition-all" :disabled="logForm.processing">
                ...
                <div v-if="(selectedItem.maintenance_logs || selectedItem.maintenanceLogs)?.length > 0" class="space-y-4">
                    <div v-for="log in (selectedItem.maintenance_logs || selectedItem.maintenanceLogs)" :key="log.id" class="p-5 border rounded-3xl bg-gray-50 hover:bg-white transition-colors border-gray-100 shadow-sm">
                        ...
                        <p v-if="log.mechanic" class="text-[9px] font-bold text-indigo-400 mt-2 uppercase italic tracking-wider">
                            By: {{ log.mechanic.name }}
                        </p>
                    </div>
                </div>
                <p v-else class="text-xs italic text-gray-400 text-center py-8 bg-gray-50/50 rounded-2xl border border-dashed">No service records found for this unit.</p>
            </div>
        </div>
    </div>
</template>
