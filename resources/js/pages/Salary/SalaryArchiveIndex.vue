    ...
    <div class="min-h-screen bg-gray-100 p-6 font-sans text-slate-900">
        <div class="max-w-6xl mx-auto">
            ...
            <div class="mb-10 text-center md:text-left">
                ...
                <p class="text-[10px] font-bold text-slate-400 uppercase tracking-[0.4em] mt-2">Historical Records for
                    {{ selectedYear }}</p>
            </div>
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
                <div v-for="record in history" :key="record.id"
                    ...
                    <div class="p-10 flex-1">
                        ...
                        <div class="space-y-4">
                            <div>
                                <p class="text-[8px] font-black text-slate-400 uppercase italic mb-1 tracking-widest">
                                    Net Payout</p>
                                <p class="text-2xl font-black text-slate-900 tracking-tight italic">
                                    ৳{{ record.total_payout.toLocaleString() }}
                                </p>
                            </div>

                            <div class="flex justify-between items-center pt-4 border-t border-slate-50">
                                <span
                                    class="bg-emerald-100 text-emerald-700 text-[8px] px-2 py-1 rounded font-black uppercase italic">Closed</span>
                    ...
                    <div class="flex border-t border-slate-100">
                        ...
                        <Link :href="route('salary.archive.show', { month: record.month, year: record.year })"
                            class="flex-[2] py-5 bg-slate-900 text-white text-center text-[10px] font-black uppercase tracking-[0.2em] group-hover:bg-indigo-600 transition-colors">
                            Open Statement
                        </Link>
                    </div>
                </div>
            ...
            <div v-if="history.length === 0"
                ...
                <h3 class="text-slate-400 font-black uppercase tracking-[0.3em] text-xs">No records found for {{
                    selectedYear }}</h3>
                <p class="text-slate-300 text-[10px] font-bold uppercase italic mt-2">Close a month from the live
                    dashboard to create an archive</p>
            </div>
